...
X_train = df.drop(columns=["Unnamed: 0"]).drop(columns=['CAT'])
y = df['CAT'].values
X = df['HEADER'].values
...
vect = CountVectorizer(analyzer='word', stop_words=stop_words, ngram_range=(1, 2), lowercase=True, strip_accents=None, binary=True).fit(X_train)
...
# df_test = pd.DataFrame(vect.transform(X).todense(), columns=['HEADER', 'MEDIANAME', 'WEBSITE'])
...
X_test = test_df.drop(columns=["Unnamed: 0"])#.drop(columns=['PTIME'])
df_test = pd.DataFrame(vect.transform(X_test).todense(), columns=['PTIME', 'HEADER', 'MEDIANAME', 'WEBSITE'])
# clf.predict(df_test)

clf.fit(df_test, y)
print(clf.score(df_test, y))
